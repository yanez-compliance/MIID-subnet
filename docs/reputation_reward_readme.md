...
1. For V1, `data/reputation_snapshot.json` is a file-based mock snapshot created directly on the Flask (core) machine. It is generated by a local script that follows the actual ER diagram structure (MINER, MINER_REPUTATION) until DB connectivity is ready.

2. Flask loads this file at startup and keeps it in memory as `CURRENT_REP_SNAPSHOT`. Snapshots may be regenerated manually or via cron. The in-memory structure looks like:
```json
{
...
```
3. Validators request reputation using `/reputation_request`, and Flask returns rep data plus the snapshot version. If a new snapshot is not yet available, validators continue using the last version they already fetched.
...
To keep the system stable and predictable, we use a gentle linear clamp on rep_score:
```
score_factor = clamp(rep_score, 0.8, 1.2)
```
This gives approximately ±20% adjustment around Neutral (1.0). Tier multipliers remain the primary lever of influence.
...
For snapshot loading, validators and Flask will rely on a file-based mock snapshot that accurately represents the intended DB structure (MINER_REPUTATION + MINER). If Flask cannot yet generate or serve a new snapshot, validators continue using the last known good snapshot they already fetched. Flask continues accepting all `/reward_allocation` JSON files so that when snapshot generation resumes, all pending allocations are accounted for; reputation is never forced back to Neutral by default.
...
def apply_reputation_weighting(base_reward, rep_score, rep_tier):
    # compute tier_factor
    # compute score_factor via clamp
