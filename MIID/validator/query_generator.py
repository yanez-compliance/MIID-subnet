...
class QueryGenerator:
    ...
    async def build_queries(self) -> Tuple[List[Dict[str, Any]], str, Dict[str, Any], str, int, str, int, Dict[str, Any]]:
        ...
        try:
            ...
            # Initialize fake for English-only country names
            ...
            while len(generated_names) < negative_sample_count:
                ...
                # Try to find a valid country with limited attempts (use English-only country generation)
                address = None
                country_attempts = 0
                max_country_attempts = 20  # Limit country selection attempts
                
                while country_attempts < max_country_attempts:
                    potential_address = fake_country.country()  # Use English-only fake for country names
                    if potential_address not in self.sanctioned_countries:
                        address = potential_address
                        break
                    country_attempts += 1
            ...
            while len(seed_names_with_labels) < fallback_sample_size:
                ...
                # Generate English country for address
                address = None
                country_attempts = 0
                max_country_attempts = 20  # Limit country selection attempts
                
                while country_attempts < max_country_attempts:
                    potential_address = fake_country.country()
                    if potential_address not in self.sanctioned_countries:
                        address = potential_address
                        break
                    country_attempts += 1
                ...
                # If we couldn't find a valid country, skip this attempt
                if address is None:
                    bt.logging.debug(f"🔄 No valid country found for fallback after {max_country_attempts} attempts. Skipping this attempt.")
                    continue
